...
class WhatsAppAdapter(BasePlatformAdapter, UnipileWebhookMixin):
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        # Keep explicit media requests on the deterministic catalog/outbox
        # path.  The generic LLM lane can acknowledge in prose but cannot
        # prove a multipart provider receipt.
        media_request = has_explicit_media_request(text)
        if media_request:
            logger.info(
                "[whatsapp] explicit media request deferred to deterministic media worker: %s",
                message_id,
            )
            return
