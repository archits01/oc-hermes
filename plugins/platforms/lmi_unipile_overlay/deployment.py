...
from .booking import LiveBookingService, register_adapter_booking_tools
    ...
    def __post_init__(self) -> None:
        ...
        if set(accounts) != set(SUPPORTED_CHANNELS):
            raise MediaOverlayError(
                "deployment must bind exactly one account for every supported media channel"
            )
        ...
        if set(platforms) != set(SUPPORTED_CHANNELS) or len(set(platforms.values())) != len(
            ...
            raise MediaOverlayError(
                "deployment must bind one distinct adapter platform per media channel"
            )
        object.__setattr__(
    ...
    def bind(
        ...
        *,
        ...
        provider_message_id = str(
            inbound_payload.get("provider_message_id")
            or inbound_payload.get("message_id")
            or inbound_payload.get("id")
            or ""
        ).strip()
        raw_inbound_text = inbound_payload.get("message") or inbound_payload.get("text") or ""
        if isinstance(raw_inbound_text, Mapping):
            raw_inbound_text = raw_inbound_text.get("text") or ""
        inbound_text = str(raw_inbound_text).strip()
        ...
        with self._lock:
            ...
            if existing is not None and (
                existing.channel, existing.account_id, existing.chat_id
...
def bind_verified_adapter_inbound_event(
    *,
    ...
    This is the exact call the deployment-owned messaging-channel
...
class SessionDatabaseMediaScopeResolver:
    ...
    def __init__(
        ...
        *,
        ...
        if set(self._accounts) != set(SUPPORTED_CHANNELS):
            raise MediaOverlayError("session resolver needs every configured media account")
        if set(self._platforms) != set(SUPPORTED_CHANNELS):
            raise MediaOverlayError("session resolver needs every configured adapter platform")
    ...
    def __call__(self, session_id: str) -> Mapping[str, str]:
        ...
        if origin and (origin_platform != adapter_platform or origin_chat_id != chat_id):
...
def install_deployment_media_tools(
    ...
    *,
    ...
    """Register every channel tool pair with verified session scope.
...
def construct_deployment_booking_service(
    *,
    config: MediaBridgeDeploymentConfig,
    media_module: ReviewedMediaModule,
    session_db: SessionRowReader,
    ...
    """Preflight every booking dependency without mutating the tool registry."""
    booking_path = Path(_text(booking_db_path, "booking_db_path"))
    if not booking_path.is_absolute():
        raise MediaOverlayError("deployment booking_db_path must be absolute")
    scope_resolver = SessionDatabaseMediaScopeResolver(session_db, config=config, inbound_scopes=inbound_scopes)
    def booking_scope_resolver(session_id: str) -> Mapping[str, Any]:
        scope = dict(scope_resolver(session_id))
        row = session_db.get_session(session_id)
        if not isinstance(row, Mapping):
            raise MediaOverlayError("no durable live session scope exists")
        origin = SessionDatabaseMediaScopeResolver._origin(row)
        lead_id = _text(row.get("user_id"), "session lead_id")
        origin_lead_id = str(origin.get("user_id") or "").strip()
        if origin_lead_id and origin_lead_id != lead_id:
            raise MediaOverlayError("origin lead_id does not match canonical session row")
        inbound = inbound_scopes.resolve(_text(row.get("session_key"), "gateway session_key"))
        scope.update({
            "lead_id": lead_id,
            "inbound_provider_message_id": inbound.inbound_provider_message_id,
            "inbound_text": inbound.inbound_text,
        })
        return scope
    bridge = construct_reviewed_media_bridge(config, media_module=media_module)
    provider = getattr(bridge, "provider", None)
    sender = getattr(provider, "send_offer", None)
    if not callable(sender):
        raise MediaOverlayError("reviewed provider cannot deliver exact-chat booking messages")
    try:
        import calendar_sync
    except ImportError as exc:
        raise MediaOverlayError("established calendar_sync boundary is unavailable") from exc
    def history_reader(*, channel: str, account_id: str, chat_id: str) -> list[str]:
        con = sqlite3.connect(config.crm_db_path, timeout=30)
        try:
            rows = con.execute(
                """SELECT text FROM chat_messages WHERE lower(channel)=? AND account_id=?
                   AND chat_id=? AND lower(direction) IN ('in','inbound','received')
                   AND text IS NOT NULL AND trim(text) != '' ORDER BY msg_ts,id""",
                (channel, account_id, chat_id),
            ).fetchall()
            return [str(row[0]) for row in rows]
        except sqlite3.Error as exc:
            raise MediaOverlayError("exact customer chat history is unavailable") from exc
        finally:
            con.close()
    service = LiveBookingService(
        db_path=str(booking_path), lead_db_path=config.crm_db_path,
        calendar_create=calendar_sync.create_live_meeting_event,
        calendar_persist=calendar_sync.persist_live_calendar_success,
        send_text=sender, scope_resolver=booking_scope_resolver, history_reader=history_reader,
    )
    return service


def install_deployment_booking_tools(ctx: Any, *, config: MediaBridgeDeploymentConfig, media_module: ReviewedMediaModule, session_db: SessionRowReader, inbound_scopes: VerifiedInboundMediaScopeRegistry, booking_db_path: str, service: LiveBookingService | None = None) -> dict[str, tuple[str, str]]:
    """Register booking tools after preflight has completed."""
    service = service or construct_deployment_booking_service(
        config=config, media_module=media_module, session_db=session_db,
        inbound_scopes=inbound_scopes, booking_db_path=booking_db_path,
    )
    return {channel: register_adapter_booking_tools(ctx, service, channel=channel) for channel in SUPPORTED_CHANNELS}
