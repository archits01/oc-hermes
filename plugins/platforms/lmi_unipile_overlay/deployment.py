...
from .booking import OwnerHandoffService, register_owner_handoff_tools
...
def construct_deployment_booking_service(
    *,
    ...
    """Preflight owner-handoff dependencies without mutating the tool registry."""
    ...
    def history_reader(*, channel: str, account_id: str, chat_id: str) -> list[str]:
        ...
        try:
            ...
            con.close()
    service = OwnerHandoffService(
        lead_db_path=config.crm_db_path,
        scope_resolver=booking_scope_resolver,
        history_reader=history_reader,
    )
    return service


def install_deployment_booking_tools(ctx: Any, *, config: MediaBridgeDeploymentConfig, media_module: ReviewedMediaModule, session_db: SessionRowReader, inbound_scopes: VerifiedInboundMediaScopeRegistry, booking_db_path: str, service: OwnerHandoffService | None = None) -> dict[str, tuple[str, ...]]:
    ...
    return {channel: register_owner_handoff_tools(ctx, service, channel=channel) for channel in SUPPORTED_CHANNELS}
