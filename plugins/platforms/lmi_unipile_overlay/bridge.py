...
SUPPORTED_CHANNELS = ("instagram", "linkedin", "whatsapp")
_CATALOG_KEY = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]{0,79}$")
...
def _catalog_key(args: Mapping[str, Any]) -> str:
    value = _required_text(args, "catalog_key")
    if not _CATALOG_KEY.fullmatch(value):
        raise MediaOverlayError("catalog_key is invalid")
    return value
...
class MediaOverlay:
    ...
    def __init__(self, reviewed_bridge: ReviewedMediaBridge, *, channel: str):
        ...
        if channel not in SUPPORTED_CHANNELS:
            raise MediaOverlayError("media overlay supports only Instagram, LinkedIn, or WhatsApp")
        ...
        return [
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "engagement_message_id": {
                            "type": "string",
                            "description": (
                                "Exact inbound provider message id that mentioned relevant "
                                "photos, videos, portfolio, or other visuals"
                            ),
                        },
                    ...
                    "required": [
                        "idempotency_key", "engagement_message_id", "account_id", "chat_id"
                    ],
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "catalog_key": {
                            "type": "string",
                            "description": (
                                "One reviewed catalog key relevant to the exact conversation. "
                                "The backend resolves the approved files and rejects a context mismatch."
                            ),
                        },
                    },
                    "required": [
        ...
        try:
            if tool_name == self.offer_tool_name:
                result = await _call_bridge(
                    ...
                    engagement_message_id=_required_text(
                        arguments, "engagement_message_id"
                    ),
                ...
                result = await _call_bridge(
                    ...
                    catalog_key=_catalog_key(arguments),
...
class LinkedInMediaOverlay(MediaOverlay):
    """LinkedIn-scoped media overlay facade."""

    def __init__(self, reviewed_bridge: ReviewedMediaBridge):
        super().__init__(reviewed_bridge, channel="linkedin")
