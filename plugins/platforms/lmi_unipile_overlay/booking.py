...
class LiveBookingService:
    ...
    def prepare(self, session_id: str, arguments: Mapping[str, Any]) -> dict[str, Any]:
        ...
        readback = f"Please confirm: a Google Meet invite to {email} for {arguments['date']} at {arguments['time']} {arguments['timezone']}. Reply yes to confirm."
        now, expires = (
            _iso(),
...
def register_adapter_booking_tools(
    ...
    ctx.register_tool(
        ...
        schema={
            "name": prepare_name,
            "description": "Send an exact-chat Meet read-back; never creates an event.",
            "parameters": {
                "type": "object",
                "properties": {
