"""Fail-closed, exact-chat Google Meet booking tools for LMI live messaging."""
...
class OwnerHandoffService:
    ...
    def request(self, session_id: str, arguments: Mapping[str, Any]) -> dict[str, Any]:
        ...
        missing = next((name for name in ("date", "time", "timezone")
        ...
        try:
            ...
            proven = calendar_sync.agreed_start_utc_iso(evidence)
            if not sarvam_locale.has_meeting_consent(evidence):
        ...
        try:
            ...
            )) or str(lead.get("stage") or "").lower() == "do_not_contact":
            ...
            result = meeting_handoff.create_request(
                ...
                customer_email=str(arguments.get("email") or lead.get("email") or ""),
...
def register_owner_handoff_tools(
    ...
    ctx.register_tool(
        ...
        schema={
            "name": name,
            "description": (
                "Persist the customer-requested time, send Vaibhav the exact conversation brief "
                "for manual booking, and wait for his confirmation. This does not book Calendar."
            ),
            "parameters": {
                "type": "object",
                "properties": {
