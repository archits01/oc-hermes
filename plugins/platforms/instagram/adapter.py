...
class InstagramAdapter(BasePlatformAdapter, UnipileWebhookMixin):
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        # Media delivery is owned by the deterministic recovery worker.  Do
        # not let the generic LLM path turn an explicit request into a text
        # promise; the webhook projection will wake the worker immediately.
        media_request = has_explicit_media_request(text)
        if media_request:
            logger.info(
                "[instagram] explicit media request deferred to deterministic media worker: %s",
                message_id,
            )
            return
