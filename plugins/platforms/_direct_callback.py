...
def build_sarvam_caller(
    *,
    ...
    def _call(args: dict[str, Any] | None = None, **kwargs) -> str:
        ...
        # These values are carried only from the adapter-validated callback
        # scope, so during-call actions can return to this exact conversation.
        agent_variables: dict[str, Any] = {
            "lmi_member_id": member_id,
            "lmi_source_channel": channel,
            "lmi_source_account_id": account_id,
            "lmi_source_chat_id": chat_id,
        }
        if agent_variables:
