...
- Price questions: never quote a number; offer two build options after venue/date and two
  concrete Mon-Sat 11am-7pm IST slots. Never dead-air a price ask.
...
def _load_chat_context(
    ...
    *,
    ...
    return "\n\n".join(part for part in (history, memory) if part)
...
class LinkedInAdapter(BasePlatformAdapter):
    ...
    async def _handle_voice_webhook(self, request) -> Any:
        """Authenticate and persist a Sarvam completion in the CRM ledger."""
        import sqlite3
        import sys as _sys
        ...
        webhook_config = payload.get("webhook_config") or {}
        metadata = webhook_config.get("metadata") if isinstance(webhook_config, dict) else {}
        metadata = metadata if isinstance(metadata, dict) else {}
        channel_info = payload.get("channel_info") or {}
        channel_info = channel_info if isinstance(channel_info, dict) else {}
        enriched = dict(payload)
        enriched["provider"] = "sarvam"
        enriched["agent_id"] = metadata.get("app_id") or os.getenv("SARVAM_APP_ID", "")
        enriched["agent_name"] = "LMI Sales - Ananya"
        enriched["user_phone_number"] = (
            metadata.get("user_phone_number")
            or metadata.get("phone_number")
            or enriched.get("user_phone_number")
            or ""
        )
        enriched["agent_phone_number"] = (
            channel_info.get("agent_phone_number")
            or enriched.get("agent_phone_number")
            or ""
        )
        ...
        try:
            if "/root/leadgen" not in _sys.path:
                _sys.path.insert(0, "/root/leadgen")
            import voice_dashboard_sync as _voice_sync

            canonical = _voice_sync.normalize_sarvam_attempt(enriched)
            canonical["provider"] = "sarvam"
            canonical["agent_id"] = canonical.get("agent_id") or enriched["agent_id"]
            canonical["agent_name"] = canonical.get("agent_name") or "LMI Sales - Ananya"
            if not canonical.get("id"):
                return web.Response(status=400, text="attempt_id required")
            with sqlite3.connect(_voice_sync.DB, timeout=30) as con:
                con.execute("PRAGMA busy_timeout=30000")
                _voice_sync._ensure_voice_columns(con)
                inserted = _voice_sync.upsert_execution(con, canonical)
                con.commit()
        except Exception:
            logger.exception("[sarvam] CRM completion persistence failed for %s", attempt_id)
            return web.Response(status=500, text="storage failed")

        logger.info("[sarvam] call logged: id=%s inserted=%s", attempt_id, bool(inserted))
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        # Explicit media requests must be handled by the approved-media
        # outbox/recovery path, which reconciles provider receipts.  Keep the
        # fast LLM lane for ordinary LinkedIn conversation only.
        media_request = has_explicit_media_request(text)
        if media_request:
            logger.info(
                "[linkedin] explicit media request deferred to deterministic media worker: %s",
                msg_id,
            )
            return
