...
describe('BillingSettings', () => {
  it('renders the deployed-today payload with buy controls hidden and usage rows visible', async () => {
    ...
    expect(
      screen.getByText(
        "Remote spending is off for this account — a billing admin can turn it on from the portal's Open Computer page."
      )
    ).toBeTruthy()
  ...
  it('renders logged-out as a connect card without normal account rows', async () => {
    ...
    expect(await screen.findByText('Connect your account')).toBeTruthy()
    expect(screen.getByText('Run /portal in the TUI or open the Portal to connect your account.')).toBeTruthy()
