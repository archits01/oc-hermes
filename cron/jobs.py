...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        try:
            ...
            # A manual trigger deliberately stores the exact run instant in
            # ``manual_run_at``. Keep that marker local to this due-scan so
            # recurring cron repair/lateness bookkeeping cannot mistake an
            # explicit run-now request for a stale scheduled occurrence.
